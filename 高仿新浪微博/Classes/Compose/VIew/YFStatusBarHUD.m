...
@interface YFStatusBarHUDButton : UIButton
@property (nonatomic, weak) UIActivityIndicatorView *loadingView;
@end
static const NSTimeInterval YFStatusBarHUDDuration = 0.5;


@implementation YFStatusBarHUD
static UIWindow *_window;
static YFStatusBarHUDButton *_button;
...
+ (void)initialize
{
    ...
    _button = [[YFStatusBarHUDButton alloc] init];
...
+ (void)showMessage:(NSString *)msg image:(NSString *)image
{
    ...
    // 动画
    [UIView animateWithDuration:YFStatusBarHUDDuration animations:^{
        _window.transform = CGAffineTransformIdentity;
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:YFStatusBarHUDDuration delay:1.0 options:UIViewAnimationOptionCurveLinear animations:^{
            _window.transform = CGAffineTransformMakeTranslation(0, -_window.height);
        } completion:^(BOOL finished) {
            _window.hidden = YES;
        }];
...
{
    [self showMessage:msg image:@"YFStatusBarHUD.bundle/success"];
}
...
{
    [self showMessage:msg image:@"YFStatusBarHUD.bundle/error"];
}
...
@implementation YFStatusBarHUDButton
